...
parser.add_argument('-a', '--agent', type=str, default="IPPO", help="agent type of agents in RL environment")
parser.add_argument('-w', '--world', type=str, default="sumo", choices=['cityflow','sumo'], help="simulator type")
parser.add_argument('-n', '--network', type=str, default="sumo4x4_3lane", help="network name") 
        ...
        if self.config['model'].get('graphic', True):
            ...
            if self.config['command']['world'] in ['cityflow', 'sumo']:
                if self.config['command']['world'] == 'sumo':
                    ...
                    flow_path = param['dir'] + param['convertflowFile']
                else:
                    # roadnet_path = param['dir'] + param['roadnetFile']
                    roadnet_path = param['dir'] + param['convertroadnetFile']
                    # flow_path = param['dir'] + param['flowFile']
                    flow_path = param['dir'] + param['convertflowFile']
            else:
                roadnet_path = param['road_file_addr']
            interface.Graph_World_Interface(roadnet_path, flow_path)  # register graphic parameters in Registry class
