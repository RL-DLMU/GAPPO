...
class TSCEnv(gym.Env):
    ...
    def step(self, actions, group_index):
        """
        ...
        assert len(actions) == len(group_index)
        self.world.step(group_index, actions)

        if not len(self.agents) == 1:
            # obs = [self.agents[index].get_ob() for index in group_index]
            # rewards = [self.agents[index].get_reward() for index in group_index]
