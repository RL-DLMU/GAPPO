...
class LaneVehicleGenerator(BaseGenerator):
    ...
    def __init__(self, world, I, fns, in_only=False, average=None, negative=False):
        ...
        if in_only:
            roads = I.in_roads  # 获得的是入车道
        else:
            roads = I.roads  # 获得入车道和出车道
        ...
        if isinstance(world, world_sumo.World):
            ...
            lanes = [item for sublist in self.lanes for item in sublist]
            phase_available_lanes = self.world.intersections[self.world.id2idx[self.I.id]].phase_available_lane
            for ph in phase_available_lanes:
                positions = [lanes.index(item) if item in lanes else -1 for item in ph]  # 找到和每个相位相关的那些车道，在lanes中分别在哪些位置，因为输入到神经网络中是将lanes都输入了的，然后通过positions就可以找到哪些是与phase相关的lane
                self.lanes_pos.append(positions)
...
##############################################################################################################################
                    # road_result = np.array([road_result[0], np.sum(road_result[1:-1]), road_result[-1]])
                    if len(road_result) == 7 :
                        road_result = np.array([np.sum(road_result[0:2]), np.sum(road_result[2:-2]), np.sum(road_result[-2:])])
                    else :
                        road_result = np.array([road_result[0], np.sum(road_result[1:-1]), road_result[-1]])
####################################################################                    
