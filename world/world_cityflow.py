...
class Intersection(object):
    ...
    def __init__(self, intersection, world):
        ...
        for moves in self.phase_available_lanelinks:
            phase_available_lanes = []
            for move in moves:
                if move[0] not in phase_available_lanes:
                    phase_available_lanes.append(move[0])
                if move[1] not in phase_available_lanes:
                    phase_available_lanes.append(move[1])
            self.phase_available_lane.append(phase_available_lanes)
        self.reset()
...
class World(object):
    ...
    def __init__(self, cityflow_config, thread_num, **kwargs):
        ...
        self.info_functions = {
            ...
            "lane_count_pressure": self.get_lane_count_pressure,
            "lane_waiting_pressure": self.get_lane_waiting_pressure,
    ...
    def get_lane_count_pressure(self):
        '''
        get_pressure
        ...
        '''
        vehicles_now = self.eng.get_lane_vehicle_count()
        if not hasattr(self, 'last_lane_count'):
            self.last_lane_count = self.eng.get_lane_vehicle_count()  # 检查vehicles_now变会不会影响last_lane_count
        vehicles = {}
        for lane in vehicles_now:
            vehicles[lane] = self.last_lane_count[lane] - vehicles_now[lane]
        self.last_lane_count = self.eng.get_lane_vehicle_count()
        return vehicles

    def get_lane_waiting_pressure(self):
        '''
        get_pressure
        Get pressure of each intersection.
        Pressure of an intersection equals to number of vehicles that in in_lanes minus number of vehicles that in out_lanes.
        ...
        :return pressures: pressure of each intersection
        '''
        vehicles_now = self.eng.get_lane_waiting_vehicle_count()
        if not hasattr(self, 'last_lane_waiting'):
            self.last_lane_waiting = self.eng.get_lane_waiting_vehicle_count()
        vehicles = {}
        for lane in vehicles_now:
            vehicles[lane] = self.last_lane_waiting[lane] - vehicles_now[lane]
        self.last_lane_waiting = self.eng.get_lane_waiting_vehicle_count()
        return vehicles
    ...
    def _get_roadnet(self, cityflow_config):
        ...
        roadnet_file = os.path.join(cityflow_config["dir"], cityflow_config["convertroadnetFile"])
