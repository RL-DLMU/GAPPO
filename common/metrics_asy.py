    ...
    def __init__(self, lane_metrics, world_metrics, world, agents):
        ...
        for k in self.lane_metric_List:
            ...
            for i in range(len(agents)):
                j = np.array([0], dtype=np.float32)
                j_.append(j)
            j_ = np.array(j_)
            self.lane_metrics.update({k: j_})
        self.lane_metrics['rewards'] = np.array([0], dtype=np.float32)
        ...
        self.lane_metrics.update({k: np.array([0], dtype=np.float32) for k in self.lane_metric_List})
        self.world_metrics = world_metrics

    def update(self, group_index, rewards=None, rewards_p=None):
        '''
        update
        Recalculate metrics.
        ...
        if rewards is not None:
            self.lane_metrics['rewards'][group_index] += np.squeeze(rewards)[:, np.newaxis]
        ...
        if 'delay' in self.lane_metrics.keys():
            ...
            self.lane_metrics['delay'][group_index] += (np.stack(np.array([self.agents[index].get_delay() for index in group_index], dtype=np.float32)))[:, np.newaxis]
        if 'queue' in self.lane_metrics.keys():
            ...
            self.lane_metrics['queue'][group_index] += (np.stack(np.array([self.agents[index].get_queue() for index in group_index], dtype=np.float32)))[:, np.newaxis]
    ...
    def delay(self):
        ...
        # apx_delay
            ...
            try:
                ...
                return np.sum(result) / (self.decision_num/3 * len(self.world.intersections))
            except KeyError:
